...
6. Run `Auto_Modificator_llvm_x64.exe` and check the boxes for the features or modifications you want to apply.
7. Close the program, open it again to make sure your checkboxes are saved, and then close it one more time.
...
* Patch to EPPL Mod (remove piston push limit)
* Patch The Overworld height limit (2048)
* Patch The End height limit (2048)
* Patch the Nether height limit (2048)
* Patch to remove the /fill command limit
* Patch to remove the /tickingarea command limits
* Patch to remove the /setmaxplayers command limit
* **Patch to disable new play screen**
...
6. Запустите `Auto_Modificator_llvm_x64.exe` и отметьте галочками те пункты и модификации, которые вам необходимы.
7. Закройте программу, откройте её заново, чтобы убедиться, что галочки включены (сохранились), и после проверки снова закройте её.

### Список поддерживаемых версий конфигов
